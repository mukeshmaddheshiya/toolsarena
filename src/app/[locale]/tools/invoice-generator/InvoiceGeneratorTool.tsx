...
import {
  ...
  Palette, Eye, ChevronDown, Save, RotateCcw, Copy,
} from 'lucide-react';
...
export function InvoiceGeneratorTool() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // ---- Header accent bar (modern & classic) ----
      ...
      for (let i = 0; i < inv.items.length; i++) {
        ...
        if (y < 180) {
          ...
          // Continue on new page - simplified
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      // Reuse same generation logic but open in new window
      // For simplicity, generate and open in iframe
      await generatePDF();
    } finally {
      setGenerating(false);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select
            value={inv.template}
            onChange={(e) => set('template', e.target.value as InvoiceData['template'])}
            className="appearance-none pl-3 pr-8 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 cursor-pointer focus:ring-2 focus:ring-primary-500 outline-none"
          >
        ...
        {/* Accent color */}
        <label className="flex items-center gap-2 px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-pointer">
        ...
        {/* Save / Reset */}
        <button onClick={saveTemplate} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
          <Save className="w-3.5 h-3.5" />
          Save Template
          {savedMsg && <span className="text-green-600 dark:text-green-400 ml-1">{savedMsg}</span>}
        </button>
        <button onClick={resetForm} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-600 dark:text-slate-300 transition-colors">
      ...
      {/* Mobile tabs */}
      <div className="flex lg:hidden rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
        <button onClick={() => setActiveTab('edit')} className={cn('flex-1 py-2.5 text-sm font-medium transition-colors', activeTab === 'edit' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300')}>
          <FileText className="w-4 h-4 inline mr-1.5" />Edit
        </button>
        <button onClick={() => setActiveTab('preview')} className={cn('flex-1 py-2.5 text-sm font-medium transition-colors', activeTab === 'preview' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300')}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={cn('space-y-6', activeTab === 'preview' && 'hidden lg:block')}>
              ...
              <div className="flex items-center gap-3">
                {inv.logoDataUrl ? (
                  <div className="relative group">
                    <img src={inv.logoDataUrl} alt="Logo" className="w-16 h-16 object-contain rounded-lg border border-slate-200 dark:border-slate-700 bg-white" />
                  ...
                  <button onClick={() => logoInputRef.current?.click()} className="w-16 h-16 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 flex flex-col items-center justify-center text-slate-400 hover:border-primary-400 hover:text-primary-500 transition-colors">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            {/* Header */}
            <div className="hidden sm:grid grid-cols-[24px_1fr_80px_100px_100px_32px] gap-2 mb-2 px-1">
              <div />
            ...
            <div className="space-y-2">
              {inv.items.map((item, idx) => (
                <div
                  ...
                  onDragEnd={onDragEnd}
                  className={cn(
                    'grid grid-cols-1 sm:grid-cols-[24px_1fr_80px_100px_100px_32px] gap-2 items-center p-2 rounded-lg border border-slate-100 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-colors',
            ...
            <button onClick={addItem} className="mt-3 flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-dashed border-slate-300 dark:border-slate-600 hover:border-primary-400 hover:text-primary-600 text-slate-500 dark:text-slate-400 transition-colors w-full justify-center">
        ...
        <div className={cn('lg:sticky lg:top-20 lg:self-start', activeTab === 'edit' && 'hidden lg:block')}>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={generatePDF}
                  disabled={generating}
                  className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white transition-colors disabled:opacity-50"
                >
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-900">
              <div
                className="bg-white shadow-lg rounded-lg mx-auto overflow-hidden"
              ...
              >
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      {inv.logoDataUrl && (
                        <img src={inv.logoDataUrl} alt="Logo" className="w-12 h-12 object-contain" />
                      )}
                      <div>
                        <div className="font-bold text-base" style={{ color: inv.accentColor }}>INVOICE</div>
                        {inv.businessName && <div className="text-[10px] text-slate-500">{inv.businessName}</div>}
                      </div>
                    </div>
                    <div className="text-right text-[10px] text-slate-500 space-y-0.5">
                      <div className="font-semibold text-slate-700">{inv.invoicePrefix}{inv.invoiceNumber}</div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-600 space-y-0.5">
                        {inv.businessName && <div className="font-medium text-slate-800">{inv.businessName}</div>}
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-600 space-y-0.5">
                        {inv.clientName && <div className="font-medium text-slate-800">{inv.clientName}</div>}
                  ...
                  {/* Items Table */}
                  <div>
                    <div className="grid grid-cols-[1fr_50px_70px_70px] gap-1 px-2 py-1.5 rounded-t text-white text-[10px] font-semibold" style={{ backgroundColor: inv.accentColor }}>
                      <div>Description</div>
                    ...
                    {inv.items.map((item, idx) => (
                      <div key={item.id} className={cn('grid grid-cols-[1fr_50px_70px_70px] gap-1 px-2 py-1.5 text-[10px]', idx % 2 === 0 ? 'bg-slate-50' : 'bg-white')}>
                        <div className="text-slate-700 truncate">{item.description || '-'}</div>
                        <div className="text-right text-slate-600">{item.quantity}</div>
                        <div className="text-right text-slate-600">{fmtMoney(item.rate, inv.currency)}</div>
                        <div className="text-right font-medium text-slate-800">{fmtMoney(item.quantity * item.rate, inv.currency)}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-end">
                    <div className="w-52 space-y-1 text-[10px]">
                  ...
                  {(inv.notes || inv.paymentTerms) && (
                    <div className="border-t border-slate-100 pt-3 space-y-2 text-[10px] text-slate-500">
                      {inv.notes && (
                        <div>
                          <div className="font-semibold text-slate-600 mb-0.5">Notes</div>
                          <div className="whitespace-pre-wrap">{inv.notes}</div>
                        </div>
                      )}
                      {inv.paymentTerms && (
                        <div>
                          <div className="font-semibold text-slate-600 mb-0.5">Payment Terms</div>
          ...
          <div className="mt-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                onClick={generatePDF}
                disabled={generating}
                className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors"
              >
