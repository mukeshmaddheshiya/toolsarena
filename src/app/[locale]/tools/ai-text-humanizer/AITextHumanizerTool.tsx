...
import { Copy, Check, RefreshCw, Sparkles, Shield, Zap, Eye, Bot, User, ArrowRight, BarChart3, Cpu, Loader2, Download } from 'lucide-react';
...
/* ═══════════════════════════════════════════════════════════════════
   RULE-BASED ENGINE (instant mode — kept from original)
   ═══════════════════════════════════════════════════════════════════ */
...
const WORD_SYNONYMS: Record<string, string[]> = {
  'utilize': ['use', 'work with', 'rely on'], 'utilizes': ['uses', 'works with', 'relies on'],
  'utilizing': ['using', 'working with', 'relying on'], 'utilization': ['use', 'usage'],
  'leverage': ['use', 'tap into', 'take advantage of'], 'leverages': ['uses', 'taps into'],
  'leveraging': ['using', 'tapping into', 'taking advantage of'],
  'facilitate': ['help', 'make easier', 'enable'], 'facilitates': ['helps', 'enables'],
  'implement': ['set up', 'build', 'put in place'], 'implements': ['sets up', 'builds'],
  ...
  'seamless': ['smooth', 'easy', 'effortless'], 'seamlessly': ['smoothly', 'easily'],
  ...
  'transform': ['change', 'reshape', 'shake up'], 'transforms': ['changes', 'reshapes'],
  ...
  'foster': ['encourage', 'build', 'grow'], 'fosters': ['encourages', 'builds'],
  'navigate': ['handle', 'deal with', 'work through'],
  'enhance': ['improve', 'boost', 'level up'], 'enhances': ['improves', 'boosts'],
  'optimize': ['improve', 'fine-tune', 'speed up'], 'optimizes': ['improves', 'fine-tunes'],
  ...
  'demonstrate': ['show', 'prove', 'make clear'], 'demonstrates': ['shows', 'proves'],
  ...
  'delve': ['dig', 'look', 'dive'], 'delves': ['digs', 'looks', 'dives'],
  'ensure': ['make sure', 'see to it', 'check that'], 'ensures': ['makes sure'],
...
const CONTRACTIONS: [RegExp, string][] = [
  ...
  [/\byou are\b/gi, "you're"], [/\bthey have\b/gi, "they've"], [/\bwe have\b/gi, "we've"],
  [/\bthey will\b/gi, "they'll"], [/\bwe will\b/gi, "we'll"], [/\byou will\b/gi, "you'll"],
  [/\bhe is\b/gi, "he's"], [/\bshe is\b/gi, "she's"],
];

const SENTENCE_STARTERS = [
  'Honestly, ', 'Look, ', 'The thing is, ', "Here's the deal - ", 'So basically, ',
  'In my experience, ', 'From what I\'ve seen, ', 'Truth be told, ',
  'If you ask me, ', 'I think ', 'To be fair, ', 'You know, ',
];
...
function humanizeRuleBased(text: string, intensity: Intensity, seed: number): string {
  if (!text.trim()) return '';
  let result = text;
  ...
  for (const [pattern, replacements] of PHRASE_REPLACEMENTS) {
    result = result.replace(pattern, (match) => {
      const r = pick(replacements, match + seed);
      return match[0] === match[0].toUpperCase() ? r.charAt(0).toUpperCase() + r.slice(1) : r;
    });
  }

  // 2. Word-level synonyms
  result = result.replace(/\b[a-zA-Z][-a-zA-Z]*\b/g, (word) => {
    ...
    if (WORD_SYNONYMS[lower]) {
      ...
      if ((hash(word + seed) % 100) / 100 < threshold) {
        const r = pick(WORD_SYNONYMS[lower], word + seed);
        return word[0] === word[0].toUpperCase() ? r.charAt(0).toUpperCase() + r.slice(1) : r;
      }
    }
    return word;
  });
  ...
  const processed = sentences.map((sentence, i) => {
    let s = sentence;
    if (level >= 2) {
      s = s.replace(/^(?:Additionally|Furthermore|Moreover|Consequently|Subsequently|Hence|Thus|Therefore|Notably|Specifically|Accordingly|Essentially|Fundamentally),?\s*/i, () => pick(TRANSITIONS, s + i + seed));
    }
    if (level >= 3) {
      ...
      if (words.length > 28) {
        ...
        for (let j = mid - 6; j <= mid + 6 && j < words.length; j++) {
          if (j > 0 && (words[j - 1].endsWith(',') || ['which', 'and', 'while'].includes(words[j].toLowerCase()))) {
            if (words[j - 1].endsWith(',')) words[j - 1] = words[j - 1].replace(/,$/, '.');
            else { words.splice(j, 0, '.'); }
            if (j + 1 < words.length) words[j + 1] = words[j + 1].charAt(0).toUpperCase() + words[j + 1].slice(1);
            s = words.join(' ');
            break;
          }
        }
  ...
  if (level >= 4) {
    result = result.replace(/, (?=which|but|and|or|though)/g, (m) => hash(m + seed) % 3 === 0 ? ' - ' : m);
  }
  return result.replace(/\s{2,}/g, ' ').replace(/\.\s*\./g, '.').replace(/,\s*,/g, ',').replace(/\n{3,}/g, '\n\n').trim();
}
...
function getAIScore(text: string): number {
  ...
  if (!/(?:'|'|')(?:s|t|re|ve|ll|d|m)\b/.test(text) && text.length > 100) score += 10;
  score += sentences.filter(s => s.trim().split(/\s+/).length > 25).length * 3;
  ...
  const humanMarkers = ["i'm", "i've", "i'll", "don't", "doesn't", "can't", "won't", "honestly", "basically", "you know", "kind of", "sort of", "pretty much", "the thing is", "tbh", "imo"];
  for (const marker of humanMarkers) { if (lower.includes(marker)) score -= 4; }
  score -= sentences.filter(s => s.trim().split(/\s+/).length < 6).length * 2;
...
async function loadModel(onProgress?: (msg: string) => void): Promise<void> {
  ...
  onProgress?.('Loading AI model (~250MB, cached after first use)...');

  const { pipeline } = await import('@huggingface/transformers');
  pipelineInstance = await pipeline('text2text-generation', 'Xenova/flan-t5-small', {
...
async function humanizeWithAI(
  text: string,
  intensity: Intensity,
  onChunkDone?: (progress: number) => void,
): Promise<string> {
  if (!pipelineInstance) throw new Error('Model not loaded');

  const intensityPrompt: Record<Intensity, string> = {
    light: 'Paraphrase the following text slightly to sound more natural:',
    medium: 'Rewrite the following text to sound conversational and human-written:',
    heavy: 'Completely rewrite the following text in a casual, natural human voice:',
    aggressive: 'Rewrite this text as if a real person is talking casually to a friend:',
  };

  // Split into sentences for better results (T5-small has ~512 token limit)
  const sentences = text.match(/[^.!?]+[.!?]+/g) || [text];
  const chunks: string[] = [];
  let current = '';

  for (const s of sentences) {
    if ((current + s).split(/\s+/).length > 60) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    const prompt = `${intensityPrompt[intensity]} ${chunks[i]}`;
    const output = await pipelineInstance(prompt, {
      max_new_tokens: 256,
      temperature: intensity === 'aggressive' ? 0.9 : intensity === 'heavy' ? 0.8 : 0.7,
      do_sample: true,
      top_p: 0.92,
    });
    results.push(output[0]?.generated_text || chunks[i]);
    onChunkDone?.(Math.round(((i + 1) / chunks.length) * 100));
  }

  let result = results.join(' ');

  // Apply contractions (AI model might not use them)
  for (const [pattern, replacement] of CONTRACTIONS) {
    result = result.replace(pattern, (match) => {
      if (match[0] === match[0].toUpperCase() && match[1] === match[1].toLowerCase()) {
        return replacement.charAt(0).toUpperCase() + replacement.slice(1);
...
export function AITextHumanizerTool() {
  ...
  // Instant mode output (rule-based, real-time)
  const instantOutput = useMemo(() => humanizeRuleBased(input, intensity, version), [input, intensity, version]);

  // Active output depends on mode
  const output = mode === 'ai' && aiOutput ? aiOutput : instantOutput;
  ...
  const loadAIModel = useCallback(async () => {
    ...
    setAiError('');
    try {
      await loadModel((msg) => setAiStatus(msg));
      setModelLoaded(true);
    } catch (e) {
      setAiError(`Failed to load AI model: ${(e as Error).message}`);
    }
  ...
  const runAI = useCallback(async () => {
    ...
    try {
      const result = await humanizeWithAI(input, intensity, (pct) => setAiProgress(pct));
      setAiOutput(result);
    } catch (e) {
      setAiError(`AI processing failed: ${(e as Error).message}`);
    }
    setAiLoading(false);
  }, [input, intensity, modelLoaded, loadAIModel]);

  // Auto-load model when switching to AI mode
  useEffect(() => {
    if (mode === 'ai' && !modelLoaded && !modelLoading) {
      loadAIModel();
    }
  }, [mode, modelLoaded, modelLoading, loadAIModel]);
  ...
  return (
    <div ref={toolRef} className="space-y-5">
      ...
      <div className="flex gap-1 bg-slate-100 dark:bg-slate-900 rounded-xl p-1">
        <button onClick={() => setMode('instant')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-semibold transition-all ${
            mode === 'instant' ? 'bg-white dark:bg-slate-700 shadow text-purple-600 dark:text-purple-400' : 'text-slate-500 hover:text-slate-700'
          }`}>
          <Zap className="w-4 h-4" /> Instant Mode
        </button>
        <button onClick={() => setMode('ai')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-semibold transition-all ${
            mode === 'ai' ? 'bg-white dark:bg-slate-700 shadow text-purple-600 dark:text-purple-400' : 'text-slate-500 hover:text-slate-700'
          }`}>
          <Cpu className="w-4 h-4" /> AI Mode
          {!modelLoaded && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400">New</span>}
        </button>
      </div>

      {/* AI Mode info banner */}
      {mode === 'ai' && (
        <div className="p-3 rounded-xl bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800 text-sm">
            ...
            <div>
              {modelLoading && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-purple-700 dark:text-purple-300 text-xs">{aiStatus || 'Loading AI model...'}</span>
                </div>
              )}
              {modelLoaded && !modelLoading && (
                <span className="text-purple-700 dark:text-purple-300 text-xs flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-green-500 inline-block"></span>
                  AI Model loaded — runs 100% in your browser, nothing sent to any server
                </span>
              )}
              {!modelLoaded && !modelLoading && (
                <div className="space-y-1">
                  <p className="text-purple-700 dark:text-purple-300 text-xs">
                    ...
                    Uses Flan-T5 AI model (~250MB download, cached after first use). Runs entirely in your browser.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { icon: Shield, label: '100% Private', desc: mode === 'ai' ? 'AI runs in your browser' : 'Nothing leaves your browser' },
          { icon: mode === 'ai' ? Cpu : Zap, label: mode === 'ai' ? 'Real AI' : 'Instant', desc: mode === 'ai' ? 'Flan-T5 model' : 'Real-time processing' },
          { icon: Eye, label: 'AI Score', desc: 'Built-in detection meter' },
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {INTENSITIES.map(m => (
            <button key={m.id} onClick={() => { setIntensity(m.id); if (mode === 'ai') setAiOutput(''); }}
              ...
              <div className={`text-lg font-black ${intensity === m.id ? m.color : 'text-slate-300 dark:text-slate-600'}`}>{m.emoji}</div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
              <Bot className="w-3.5 h-3.5 text-red-500" /> AI-Generated Text
            </span>
            <span className="text-[10px] text-slate-400">{inputWords} words</span>
          </div>
          <textarea value={input} onChange={e => { setInput(e.target.value); if (mode === 'ai') setAiOutput(''); }}
            placeholder="Paste your AI-generated text here (from ChatGPT, Gemini, Claude, etc.)..."
            rows={14}
            className="w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none resize-none leading-relaxed placeholder:text-slate-400 dark:text-slate-200" />
          {input.trim() && (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
              <User className="w-3.5 h-3.5 text-green-500" /> Humanized Text
              ...
              {mode === 'ai' && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold text-purple-600 bg-purple-100 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-800">AI</span>}
            </span>
            <div className="flex items-center gap-2">
          ...
          <textarea readOnly value={aiLoading ? '' : output}
            placeholder={aiLoading ? 'AI is processing...' : 'Humanized text will appear here...'}
            rows={14}
          ...
          {/* AI progress */}
          {aiLoading && (
            <div className="flex items-center gap-3 p-3 rounded-xl bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800">
              <Loader2 className="w-4 h-4 animate-spin text-purple-600 shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-medium text-purple-600">AI Processing</span>
                  <span className="text-[10px] font-bold text-purple-600">{aiProgress}%</span>
                </div>
                <div className="w-full h-1.5 bg-purple-200 dark:bg-purple-800 rounded-full overflow-hidden">
                  <div className="h-full bg-purple-600 rounded-full transition-all duration-300" style={{ width: `${aiProgress}%` }} />
                </div>
              </div>
            </div>
          )}
          ...
          {/* AI error */}
          {aiError && (
            <p className="text-sm text-red-600 dark:text-red-400 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">{aiError}</p>
          )}

          {/* Action buttons */}
          <div className="flex flex-wrap gap-2">
            {mode === 'ai' && (
              <button onClick={runAI} disabled={aiLoading || !input.trim()}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-purple-600 text-white hover:bg-purple-700 shadow-md shadow-purple-600/20 transition-all disabled:opacity-40 disabled:shadow-none">
                {aiLoading ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Processing...</> : <><Cpu className="w-3.5 h-3.5" /> Humanize with AI</>}
              </button>
            )}
            <button onClick={copy} disabled={!output || aiLoading}
              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                copied
                  ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400'
                  : mode === 'ai' ? 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-300' : 'bg-purple-600 text-white hover:bg-purple-700 shadow-md shadow-purple-600/20'
              } disabled:opacity-40 disabled:shadow-none`}>
              {copied ? <><Check className="w-3.5 h-3.5" /> Copied!</> : <><Copy className="w-3.5 h-3.5" /> Copy Text</>}
            </button>
            {mode === 'ai' && aiOutput && (
              <button onClick={() => { setAiOutput(''); runAI(); }}
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-500 hover:text-purple-600 transition-colors">
      ...
      {input && output && !aiLoading && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/30 rounded-xl p-3 text-center">
            ...
            <div className="text-2xl font-black text-purple-600">{scoreReduced > 0 ? `-${scoreReduced}` : '0'}%</div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 text-center">
            ...
            <div className="text-2xl font-black text-slate-700 dark:text-slate-300">{wordChange > 0 ? `+${wordChange}` : wordChange}%</div>
            <div className="text-[10px] font-bold text-slate-500">{outputWords >= inputWords ? 'Words added' : 'Words reduced'}</div>
          </div>
        </div>
      )}
    </div>
